...
const extensionInjectionMarkerPattern = /grammarly|immersive[-_ ]?translate|language[-_ ]?tool|deepl|quillbot|prowritingaid|microsoft[-_ ]?editor|resume-editor-chrome|extension|plugin|overlay|toolbar|translate|notranslate|trans[-_]|gtx|ydd|youdao|caiyun|saladict|dict[-_]|dictcontainer|monica|sider|chatgpt|copilot|sidebar|tooltip|bubble|shadow[-_]root|shadow[-_]host|crx|tampermonkey|violentmonkey|lastpass|bitwarden|1password|autofill|darkreader|stylish|stylus|ublock|adblock/i;
...
function normalizeSubmittedHtml(sourceDocument, submittedDocument, submittedHtml) {
  ...
  const sourceHeadElements = headElementCounts(sourceDocument);
  const actions = [];

  walkForSaveNormalization(submittedDocument, (node, parent, editableText) => {
    ...
    if (parent?.tagName === "head") {
      ...
      const key = headElementKey(node);
      const remaining = sourceHeadElements.get(key) || 0;
      if (remaining > 0) {
        sourceHeadElements.set(key, remaining - 1);
      } else {
...
function walkForSaveNormalization(node, callback, parent = undefined, editableText = false) {
  if (node.tagName) {
    callback(node, parent, editableText);
    editableText = editableText || Boolean(attribute(node, "data-resume-editor-id"));
  }
  for (const child of node.childNodes || []) walkForSaveNormalization(child, callback, node, editableText);
  if (node.content) walkForSaveNormalization(node.content, callback, node, editableText);
}
...
function isExtensionInjectedNode(node) {
  if (!node.tagName || ["html", "head", "body"].includes(node.tagName) || attribute(node, "data-resume-editor-id")) return false;
...
function isNonSemanticWrapper(node) {
  ...
  const marker = attributes.flatMap(({ name, value }) => [name, value]).join(" ");
  if (extensionInjectionMarkerPattern.test(marker)) return true;
  if (node.tagName === "font") return true;
  if (attributes.some(({ name }) => name === "translate" || name.startsWith("data-") || editorRuntimeInjectedAttributeNameSet.has(name))) {
    return true;
  }
...
function findHeadNode(document) {
  let head = null;
  visit(document, (node) => {
...
function headElementKey(node) {
  if (!node.tagName) return "";
  const attrs = (node.attrs || [])
    .filter(({ name }) => !editorRuntimeInjectedAttributeNameSet.has(name) && !name.startsWith("data-resume-editor-"))
    .map(({ name, value }) => `${name}=${value}`)
    .sort()
    .join(";");
  const text = textContent(node).trim().replace(/\s+/g, " ");
  return `${node.tagName}\u0000${attrs}\u0000${text}`;
}

function headElementCounts(document) {
  const head = findHeadNode(document);
  if (!head) return new Map();
  const counts = new Map();
  for (const child of head.childNodes || []) {
    if (!child.tagName) continue;
    if (child.tagName === "style" && attribute(child, "id") === "resume-editor-overrides") continue;
    const key = headElementKey(child);
    counts.set(key, (counts.get(key) || 0) + 1);
  }
  return counts;
}
...
function assertEditableDocument(sourceNode, submittedNode, editableText = false) {
  if (sourceNode.nodeName !== submittedNode.nodeName) {
    throw new Error(`Canvas 只能编辑已有文字，不能修改简历结构（节点不匹配：期望 <${sourceNode.nodeName}>，实际 <${submittedNode.nodeName}>）。`);
  }
  ...
  const sourceChildren = comparableChildren(sourceNode);
  const submittedChildren = comparableChildren(submittedNode);
  if (sourceChildren.length !== submittedChildren.length) {
    const context = describeElement(sourceNode);
    throw new Error(`Canvas 只能编辑已有文字，不能修改简历结构（在 ${context} 中子元素数量不匹配：期望 ${sourceChildren.length} 个，实际 ${submittedChildren.length} 个）。`);
  }
...
function comparableChildren(node) {
  ...
  return children.filter((child) => {
    if (child.nodeName === "#documentType") return false;
    if (child.tagName && isExtensionInjectedNode(child)) return false;
    if (node.tagName === "head") {
      if (child.nodeName === "#text" && !child.value.trim()) return false;
      if (child.tagName === "style" && attribute(child, "id") === "resume-editor-overrides") return false;
    }
    return true;
  });
}
...
function firstAttributeDifference(sourceNode, submittedNode) {
  ...
  for (const name of names) {
    ...
    if (["html", "head"].includes(sourceNode.tagName) && name.startsWith("data-") && !sourceAttrs.has(name)) continue;
    if (name === "style") {
