...
const extensionInjectionMarkerPattern = /grammarly|immersive[-_ ]?translate|language[-_ ]?tool|deepl|quillbot|prowritingaid|microsoft[-_ ]?editor|resume-editor-chrome|extension|plugin|overlay|toolbar/i;
...
function comparableChildren(node) {
  ...
  return node.nodeName === "#document" ? children.filter((child) => child.nodeName !== "#documentType") : children;
}
