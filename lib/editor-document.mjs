...
function normalizeSubmittedHtml(sourceDocument, submittedDocument, submittedHtml) {
  const sourceDataAttributes = dataAttributeCounts(sourceDocument);
  ...
  walkForSaveNormalization(submittedDocument, (node, editableText) => {
    ...
    if (inEditableField && isDeterministicallyHidden(node)) {
    ...
    for (const attr of node.attrs || []) {
      if (!shouldNormalizeAttribute(attr.name, node, inEditableField, sourceDataAttributes)) continue;
...
function shouldNormalizeAttribute(name, node, inEditableField, sourceDataAttributes) {
  if (editorRuntimeInjectedAttributeNameSet.has(name)) return true;
  if (!inEditableField || !name.startsWith("data-")) return false;
  ...
  const remaining = sourceDataAttributes.get(key) || 0;
  if (remaining > 0) {
    sourceDataAttributes.set(key, remaining - 1);
    return false;
  }
  return true;
}
...
function isNonSemanticWrapper(node) {
  if (node.tagName !== "span" || attribute(node, "data-resume-editor-id")) return false;
  return (node.attrs || []).every(({ name }) => editorRuntimeInjectedAttributeNameSet.has(name) || name.startsWith("data-"));
}
...
function dataAttributeCounts(document) {
...
function hiddenNodeCounts(document) {
  ...
  walkForSaveNormalization(document, (node, editableText) => {
    if (!editableText && !attribute(node, "data-resume-editor-id")) return;
...
// 返回第一个真实属性差异（或 null）。editorRuntimeInjectedAttributeNames（库内单一事实源）
// 是浏览器/画布运行时注入，完全忽略；html 根上的新增 data-* 也视为浏览器扩展噪声，
// 保存返回值时会从原始字符串中移除。style 仅在一侧出现时忽略（容忍运行时注入，源模板应
// 合法携带内联 style），两侧都有则去全部空白后比较；其余属性任何新增/缺失/变更都是真实差异。
