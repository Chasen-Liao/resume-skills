...
const editorContainerClassNames = new Set(["page", "resume"]);
const blockChildTagNames = new Set([
...
function editorFieldStructureProblem(node, markedNodes) {
  ...
  const blockChild = nonTextChildren.find((child) => blockChildTagNames.has(child.tagName));
  if (blockChild) {
...
function nestedInlineFieldProblem(node, markedNodes) {
  ...
  if (blockChildTagNames.has(node.tagName)) {
  ...
  const childNonText = (node.childNodes || []).filter((child) => child.nodeName !== "#text" && child.nodeName !== "#comment");
  if (childNonText.some((child) => blockChildTagNames.has(child.tagName))) {
    return "编辑字段不能嵌套在另一个编辑字段内部（嵌套字段含块级子元素）";
  }
  if (childNonText.length > 1) {
    return "编辑字段不能嵌套在另一个编辑字段内部（嵌套字段含多个子节点）";
  }
  return null;
}
...
function describeField(node, id) {
  const tag = node.tagName;
  const classes = (attribute(node, "class") || "").split(/\s+/).filter(Boolean);
  const prefix = classes.length ? `${tag}.${classes.join(".")}` : tag;
  const snippet = (node.childNodes || []).filter((child) => child.nodeName === "#text").map((child) => child.value).join("").trim();
  return `${prefix} data-resume-editor-id="${id}"${snippet ? `，文本“${snippet.slice(0, 40)}”` : ""}`;
}
...
export function validateEditorSave(sourceHtml, submittedHtml) {
  ...
  assertEditableDocument(sourceDocument, submittedDocument);
...
function assertEditableDocument(sourceNode, submittedNode, editableText = false) {
  ...
  if (sourceNode.tagName && attributesKey(sourceNode) !== attributesKey(submittedNode)) {
    throw new Error("Canvas 只能编辑已有文字，不能修改 HTML 属性或结构。");
  }
...
function attributesKey(node) {
  return (node.attrs || []).map(({ name, value }) => `${name}=${value}`).sort().join("\u0000");
}
