...
命令会启动本地服务并尝试打开浏览器。告知用户定制版 HTML/PDF、匹配报告和 Canvas 导出的 `<原文件名>-edited.html` 位置。若当前环境无法执行 `npx`，明确报告未启动，并提供带实际 HTML 路径的完整命令，不得声称已预览。ATS-safe 模式不启动 Canvas，只交付文件位置和浏览器打印方法。
