...
**设计定位**：在 **稳重可信** 的商务调性与 **现代低噪** 的极简美学之间取得平衡。以深蓝色（Navy）为主基调，亮蓝色（Accent）点缀，大面积白底 + 小面积浅蓝 wash。使用精致克制的细边框和虚线，替代大面积粗重色块。

**Do's & Don'ts**:
- ✅ 主色点缀鲜明：深蓝配高亮冰蓝，形成明确对比
- ✅ 虚实结合：多项目间用 dashed 虚线，Header 底部用实线
- ✅ 中英文混排优雅：标题用 Lexend/Outfit，中文用系统默认无衬线
- ❌ 避免平铺大色块（深蓝色大面积背景）
- ❌ 避免实线过多（页面网格化、死板）
- ❌ 不要混合超过 2 组以上英文字体系列
