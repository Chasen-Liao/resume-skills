...
**技能协作流转**：
- **经历打磨**：采访中若发现某条经历职责化、缺少具体贡献或证据不足，直接依据 `references/content-writing.md` 就地引导用户提供行动与交付物证据，就地改写为强动词结构，不打断采访主流程；
- **质量审计**：母版交付前，通过 `validate_resume.py`（视觉单页与密度）或 `--mode ats`（ATS 格式）执行自动化质量审计；
- **排版微调**：视觉母版生成并完成 PDF 验证后，若用户需要交互式微调，**路由至 `resume-canvas`**；
- **岗位定制**：母版定稿后若需投递具体岗位，**路由至 `jd-tailorer`**（一站式完成 JD 结构化分析、匹配评估与定向定制）；
- **版本追踪**：母版定稿后，建议用户在本地私有 Git 仓库提交初始母版（如 `git commit -m "创建母版简历"`），保护事实源。
...
视觉模式：
- A4 尺寸、页边距、字号、间距、行高等排版参数参见 `css/common.md`（唯一定义源）；根 `<html>` 必须声明 `data-resume-layout="full-page"`，并保留 `--resume-density`、`--resume-density-min`、`--resume-fill-target: 0.98` 和 `--resume-bottom-safe` 变量。
- 页面主体必须按 `css/common.md` 的 full-page 契约使用可伸展的 flex/grid 布局：内容不足时用 `space-between` 均匀分布，内容过多时由渲染脚本在安全密度下限之上自动压缩；不得用虚构内容填白或用 `overflow: hidden` 静默裁切。
...
- `page fill` 以 PDF 可提取文本的纵向范围估算**有效页面区域**占用率，目标至少 `98%`；默认低于目标直接失败并明确提示布局密度/间距问题，不能为填白编造事实。
- `vertical balance` 检查可打印区域上下留白差；出现警告时优先均匀调整板块间距、条目间距和容器内边距，避免内容挤在顶部或底部。
- `bottom safety` 不得失败；内容越过可打印底部安全区或安全余量不足时，必须回退间距或字号调整。

版面调整顺序：先使用已有且已确认的板块和内容；短内容由 full-page 契约均匀分布，长内容由渲染脚本在 `--resume-density-min` 之上自动压缩；仍溢出再按“页边距不小于 8mm → 间距 → 行高不低于 1.25 → 正文不小于 9.5px → 精简低相关内容 → 在适合时切换双栏”的顺序处理。上下间距尽量相近，任何一次调整后都重新导出 PDF 并运行同一命令。不得新增未确认经历、指标、技能或占位文本，也不以难以阅读的小字号硬塞一页。

渲染脚本会在导出 PDF 前执行 full-page 自动布局：先保持默认密度，只有检测到 A4 溢出才迭代 `--resume-density`，并同时生成同名前缀的低分辨率 `*.preview.png`。随后执行浏览器溢出和 PDF 布局验证，并生成同名前缀的 `*.resume-manifest.json`，其中包含 HTML/PDF/preview SHA-256、renderer 版本和验证结果。`degraded`、`fail` 或 hash 不一致都不可交付。完成自动检查后，再人工检查预览图中的可见裁切、断图、照片、字体回退、链接和打印背景；自动检查不能保证这些视觉细节。视觉模式以 A4 单页和有效区域 ≥98% 为硬目标，所有警告都要记录处理结果。
