...
它不占用用户的主流程入口：在母版确认、JD 定制版确认或 Canvas 微调确认后记录版本即可；用户需要比较、恢复或维护投递记录时再展示版本操作。

## 协作入口与交接

- **上游来源**：
  - `resume-builder`：母版 HTML/PDF 生成并确认后，记录母版初始版本；
  - `jd-tailorer`：针对具体岗位的定制版完成验证后，记录定制版本与 JD 映射；
  - `resume-canvas`：本地交互微调完成、且事实与 PDF 重验通过后，记录微调修订版本。
- **下游去向**：版本记录或 Git 提交完成后，提示用户可继续使用 `job-description-analyzer` / `jd-tailorer` 投递其他新岗位，或在需要时回溯对比历史版本。
