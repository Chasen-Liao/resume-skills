...
- `resume-skills editor <resume.html>` 只负责受限的排版微调，不生成或改写简历内容；正文事实必须回到 Skill 工作流确认。
...
- Canvas 仅支持选择文本节点并调整字号、字重、颜色、对齐、行高、段后距、页边距、主题色等排版项；不持久化文字编辑，不支持 AI 改写、JD 匹配、结构重排、自由拖拽或图片编辑。
...
最后一条会启动本地编辑器。验证保存时确认：排版覆盖会原子写回 HTML 并使关联 manifest 失效；文字事实不会通过 Canvas 保存。
